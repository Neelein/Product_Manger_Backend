...
func (r *AnnouncementRepositoryPGX) GetByID(ctx context.Context, id string) (*domain.Announcement, error) {
	...
	err := r.pool.QueryRow(ctx, "SELECT * FROM get_announcement_by_id($1)", id).Scan(&a.ID, &a.Title, &a.Content, &a.ImagePath, &a.PublisherID, &a.PublisherName, &a.CreatedAt, &a.UpdatedAt)
	if err != nil {
