...
func TestMain(m *testing.M) {
	databaseURL, err := testconfig.RequiredTestDatabaseURL()
	if err != nil {
		panic(err)
	}
...
func runMigration(ctx context.Context, pool *pgxpool.Pool) {
	for _, file := range []string{
		...
		"../../../db/migrations/019_create_payments.up.sql",
