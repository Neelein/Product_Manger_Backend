...
func (r *EventRepositoryPGX) GetByID(ctx context.Context, id string, memberID string) (*domain.Event, error) {
	var e domain.Event
	err := r.pool.QueryRow(ctx, "SELECT * FROM get_event_by_id($1, $2)", id, memberID,
	).Scan(&e.ID, &e.Title, &e.Description, &e.StartTime, &e.EndTime, &e.Status, &e.CreatedBy, &e.CreatorName, &e.CreatedAt, &e.UpdatedAt)
	if err != nil {
